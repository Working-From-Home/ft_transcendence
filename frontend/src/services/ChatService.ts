...
class ChatService {
	...
	muteUser(channelId:number, userId: number, muteUntil: Date): Promise<any> {
		return http.post(`/channels/${channelId}/mute/${userId}`, muteUntil)
	}
	banUser(channelId:number, userId: number, banUntil: Date): Promise<any> {
		return http.post(`/channels/${channelId}/ban/${userId}`, banUntil)
	}
